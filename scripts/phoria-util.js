...
(function() {
   ...
   Phoria.Util.generateIcosahedron = function generateIcosahedron(scale)
   {
      ...
      // note: the student web page no longer valid
   ...
   Phoria.Util.generateCylinder = function generateCylinder(radius, length, strips)
   {
      ...
      for (var s=0, offset=0; s<=strips; s++)
      {
         ...
         offset += inc;
					//s==0 created 2 points, 
					//each iteration create 2 points, polygons are defined with this 2, upper, lower...plus prev upper/lower points
					
         if (s !== 0)
         {
            // quad strip, s=1,  0, 2, 3, 1    left edge 0,1  new edge 2,3, upper lower
            polygons.push({vertices: [s*2-2, s*2, s*2+1, s*2-1]});
            
            // edges, why 3?  not 2? not 4?
            //s=1,  2,0    0,1   3,1.......  I add  2,3
            //edges.push({a:s*2, b:s*2-2},{a:s*2-2,b:s*2-1},{a:s*2+1,b:s*2-1},{a:s*2,b:s*2+1});
            
            //try redo it, should be 20, 01, 13, 32
            edges.push({a:s*2, b:s*2-2},{a:s*2-2,b:s*2-1},{a:s*2-1,b:s*2+1},{a:s*2+1,b:s*2});
            
            
           var l= [s*2, s*2-2 , s*2-2,  s*2-1, s*2+1 ,s*2-1, s*2 ,s*2+1];
            //console.log(l);
            if (s === strips - 1)
            {
               
             ////michael , at last strip, it generate end caps !!!
             //// how to map texture?
             
             ////console.log(points.length);
               // end cap polygons? just some vertice number will do? no need edges?
               
               //michael changed i=strips to strips-1, else that i*2 > points.length
               
               //this for test, demo that texture can map to a 4 pt polygon
               //more than that, fail.
              //var vs = [14, 12, 8, 6];
               
              var vs = [];
							for (var i=strips-1; i>=0; i--) {	
               		vs.push(i*2);
								}
   ...
   //generateUVs true, texture map wrap as one, else, map to each polygon seperately
   Phoria.Util.generateSphere = function generateSphere(scale, lats, longs, generateUVs)
   {
      ...
      for (var latNumber = 0; latNumber <= lats; ++latNumber)
      {
         for (var longNumber = 0; longNumber <= longs; ++longNumber)
         {
            ...
            if (generateUVs)
            {
            	//uvs just array of index numbers, and should be straight forward like this
      ...
      for (var latNumber = 0; latNumber < lats; ++latNumber)
      {
         for (var longNumber = 0; longNumber < longs; ++longNumber)
         {
            ...
            // latNumber=0, longNumber=0 =>  first=0, second = longs+1
            ...
            if (latNumber === 0)
            {
               ...
               if (generateUVs)
               {
               		//so polys now has vertices, and uvs
               		// 6 in number, for quad its 8, double the vertex , since it set u, v seperately
               		//actually it follows the vertices numbering
   ...
   //=======================michael's disc
     ...
     Phoria.Util.generateDisc = function generateDisc(scale, strips, generateUVs)
   {
      var points = [], edges = [], polys = [], uvs = [];
			
			//gen uvs n points
			//remember, we are generating triangles with a common center point 0,0
               ...
               y: 0
				...
				var pilog=[];
         for (var longNumber = 0; longNumber <= strips; ++longNumber)
         {
         
         	 		//var theta = latNumber * Math.PI / lats;            
         		
         		//actually means (longNumber / strips) * ( 2 * Math.PI )
            var phi = longNumber * ( 2 * Math.PI / strips);
            pilog.push(phi);
            	//var sinTheta = Math.sin(theta);
            var sinPhi = Math.sin(phi);
            	//var cosTheta = Math.cos(theta);
            ...
            //my math, last y not back to 0 ?!
            
            if (generateUVs)
            {
            	//so uvs just array of percentage, last value is 1/1 ?!
               var u = longNumber/strips;
               var v = 1;
               uvs.push({u: u, v: v});
            }
			...
			//gen poly n edges
			//note that 0 was reserved as origin
			
         for (var longNumber = 1; longNumber <= strips; ++longNumber)
         {
            var first = longNumber;
            var second = first + 1;
          
						  
						var p = {vertices: [ 0, first, second]};
						...
						if (generateUVs)
						{
							...
							p.uvs = [uvs[0].u, uvs[0].v, uvs[first].u, uvs[first].v, uvs[second].u, uvs[second].v]
						}
						polys.push(p);
						edges.push({a:first, b:second});            
